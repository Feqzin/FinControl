...
ALTER TABLE pessoa_saldo_movimentacoes
  ...
  ADD COLUMN IF NOT EXISTS pessoa_id varchar,
  ...
  ADD COLUMN IF NOT EXISTS comprovante_referencia text,
  ADD COLUMN IF NOT EXISTS divida_id varchar,
  ADD COLUMN IF NOT EXISTS compra_cartao_id varchar,
  ADD COLUMN IF NOT EXISTS parcela_compra_id varchar,
  ADD COLUMN IF NOT EXISTS servico_pessoa_id varchar,
...
CREATE INDEX IF NOT EXISTS idx_pessoa_saldo_mov_parcela_compra_id
  ON pessoa_saldo_movimentacoes (parcela_compra_id);
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE pessoa_saldo_movimentacoes
      ...
      CHECK (tipo IN ('credito', 'debito')) NOT VALID;
  END IF;
END $$;
