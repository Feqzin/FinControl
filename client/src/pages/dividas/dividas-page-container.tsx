...
export default function DividasPage() {
  ...
  const handleCreateParcelado = () => {
    ...
    createParceladoMutation.mutate(payload, {
      onSuccess: () => {
        setOpen(false);
        setParceladoForm({
          pessoaId: "",
        ...
        toast({ title: `Divida parcelada criada com ${payload.totalParcelas} parcelas` });
      },
      onError: (error) => {
        toast({ title: "Erro", description: getErrorMessage(error), variant: "destructive" });
      },
  ...
  const handlePagarParcela = () => {
    if (!payingParcela) return;
    payParcelaMutation.mutate(
      { id: payingParcela.id, ...payParcelaForm },
      {
        onSuccess: () => {
  ...
  const handleEditarParcela = () => {
    ...
    editParcelaMutation.mutate(
      { id: editingParcela.id, ...editParcelaForm },
      {
        onSuccess: () => {
  ...
  const handleAnteciparParcelas = () => {
    if (!anteciparDivida) return;
    anteciparMutation.mutate(
      {
      ...
      {
        onSuccess: (data) => {
          const result = data as { updated?: number; todasPagas?: boolean };
          const updated = result.updated ?? 0;
          const quitada = result.todasPagas ? " · Divida quitada!" : "";
          setAnteciparOpen(false);
          setAnteciparDivida(null);
          toast({ title: `${updated} parcela(s) antecipada(s)${quitada}` });
        },
      },
    );
  };
  ...
  const handleRecalcularParcelas = () => {
    ...
    recalcularMutation.mutate(
      ...
      {
        onSuccess: (data) => {
          const result = data as { pagas?: number; novas?: number };
          const pagas = result.pagas ?? 0;
          const novas = result.novas ?? 0;
          setEditingDivida(null);
          setShowRecalcular(false);
          toast({ title: `Parcelas recalculadas: ${pagas} pagas mantidas, ${novas} novas criadas` });
        },
        onError: (error) => {
          toast({ title: "Erro", description: getErrorMessage(error), variant: "destructive" });
        },
  ...
  const handleUpdateDivida = () => {
    if (!editingDivida) return;
    updateDividaMutation.mutate(
      {
        id: editingDivida.id,
        data: {
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <Tabs value={createTab} onValueChange={(v) => setCreateTab(v as any)}>
              ...
              <TabsContent value="simples" className="space-y-4 mt-4">
                <form onSubmit={(e) => { e.preventDefault(); handleCreateSimple(); }} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={simpleForm.formaPagamento} onValueChange={(v) => setSimpleForm({ ...simpleForm, formaPagamento: v })}>
                        ...
                        <SelectContent>{FORMAS_PAGAMENTO_DIVIDA.map((f) => <SelectItem key={f.value} value={f.value}>{f.label}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                  </div>
                  <div className="space-y-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {hasParce
                            ...
                            : status === "pago"
                            ? `Pago em ${formatDividaDate(d.dataVencimento)}`
                            : `Venc: ${formatDividaDate(d.dataVencimento)}${simpleOverdue ? " · ATRASADO" : ""}`
                          }
                    ...
                    {d.parcelas.map((p) => {
                      ...
                      return (
                        <div key={p.id} className={`flex items-center gap-3 px-4 py-2.5 text-sm ${
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-medium">{formatDividaCurrency(Number(p.valor))}</p>
                            <p className="text-xs text-muted-foreground">
                              {pago
                                ? `Pago em ${formatDividaDate(p.dataPagamento!)}`
                                : `Venc. ${formatDividaDate(p.dataVencimento)}${overdue ? " · ATRASADO" : ""}`
                              }
                            </p>
                          </div>
                          {p.status === "pendente" && (
          ...
          {anteciparDivida && (
            <div className="space-y-4">
              ...
              <Button className="w-full" data-testid="button-confirm-antecipar"
                onClick={handleAnteciparParcelas}
