...
const parcelasCompraProjectionWithoutComprovante = {
...
type ParcelaCompraWithoutComprovante = {
  id: string;
  userId: string;
...
function isMissingUsersOptionalColumnsError(error: unknown): boolean {
  ...
  const referencesUsersOptionalColumnsInFailedQuery =
...
function toUserWithOptionalDefaults(user: {
  id: string;
  username: string;
  email?: string | null;
  password: string;
...
function isMissingParcelasCompraComprovanteColumnsError(error: unknown): boolean {
  const messages: string[] = [];
  const queue: unknown[] = [error];
...
export class DatabaseStorage implements IStorage {
  ...
  private isMissingVacationPlansTableError(error: unknown): boolean {
  ...
  private async ensureVacationPlansTable(): Promise<void> {
    await this.database.execute(sql.raw(`
      ...
      CREATE INDEX IF NOT EXISTS idx_vacation_plans_user_id ON vacation_plans(user_id);
      CREATE INDEX IF NOT EXISTS idx_vacation_plans_renda_id ON vacation_plans(renda_id);
      CREATE INDEX IF NOT EXISTS idx_vacation_plans_start_date ON vacation_plans(start_date);
    `));
  }

  private async withVacationPlansTable<T>(operation: () => Promise<T>): Promise<T> {
    try {
      return await operation();
    } catch (error) {
      if (!this.isMissingVacationPlansTableError(error)) throw error;
      await this.ensureVacationPlansTable();
      return operation();
    }
  }
  ...
  async getUserByEmail(email: string) {
    ...
    writeTechnicalLog({
      event: "storage.users.get_user_by_email",
      source: "storage",
      level: "info",
      data: {
    ...
    try {
      ...
      writeTechnicalLog({
        event: "storage.users.get_user_by_email.result",
        source: "storage",
        level: "info",
        data: {
          emailReceived: normalizedEmail,
      ...
      if (!isMissingUsersOptionalColumnsError(error)) {
        writeTechnicalLog({
          event: "storage.users.get_user_by_email.error",
          source: "storage",
          level: "error",
          data: {
            emailReceived: normalizedEmail,
      ...
      writeTechnicalLog({
        event: "storage.users.get_user_by_email.fallback",
        source: "storage",
        level: "warn",
        data: {
          emailReceived: normalizedEmail,
  ...
  async createUser(insertUser: InsertUser) {
    try {
      ...
      const [user] = await this.database.insert(users).values(fallbackInsertUser).returning(usersBaseProjection);
  ...
  async getParcelasCompra(compraCartaoId: string, userId: string) {
    try {
      ...
      return rows
        .map(withParcelaCompraComprovanteDefaults)
  ...
  async getParcelaCompraById(id: string, userId: string) {
    try {
      ...
      return row ? withParcelaCompraComprovanteDefaults(row) : undefined;
    }
  }
  ...
  async getVacationPlans(userId: string) {
    return this.withVacationPlansTable<VacationPlan[]>(
      () => this.database.select().from(vacationPlans).where(eq(vacationPlans.userId, userId)),
    );
  }
  async createVacationPlan(data: InsertVacationPlan) {
    const [row] = await this.withVacationPlansTable<VacationPlan[]>(
